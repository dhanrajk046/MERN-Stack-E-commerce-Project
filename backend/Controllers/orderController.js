  ...
  try {
    ...
    for (const item of items) {
      ...
      if (!item.product || !Number.isInteger(quantity) || quantity < 1) {
        return res.status(400).json({ message: 'Each item needs a product and a valid quantity' });
      }
    ...
    const itemSummary = orderItems
      .map((item) => `- ${item.name} x ${item.quantity}: INR ${(item.price * item.quantity).toFixed(2)}`)
      .join('\n');
    ...
    if (!emailSent) {
      console.warn(`Order confirmation email was not sent for order ${order._id}`);
    }
